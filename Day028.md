...
```
A third argument, when provided, specifies the minimum dimension below which 
the manufacturer cannot produce tiles. If and when that constraint is specified, 
the solution must return the next best optimum size of the tile accordingly. 
```
